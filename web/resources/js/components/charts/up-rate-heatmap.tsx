...
function cellColor(upRate: number, total: number): string {
    ...
    if (upRate >= 65) return 'bg-emerald-700 text-white';
    if (upRate >= 55) return 'bg-emerald-600 text-white';
    if (upRate >= 50) return 'bg-emerald-400 text-emerald-950';
    if (upRate >= 45) return 'bg-red-400 text-red-950';
    if (upRate >= 35) return 'bg-red-600 text-white';
                    ...
                    {MOVE_BINS.map((mb) => (
                        <tr key={mb}>
                            ...
                            {timeBins.map((tb) => {
                                ...
                                const upRate = cell ? cell.win_rate * 100 : 0;
