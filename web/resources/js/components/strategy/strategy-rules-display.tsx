...
export function isFormModeGraph(graph: unknown): graph is FormModeGraph {
    return (
        typeof graph === 'object' &&
        graph !== null &&
        'mode' in graph &&
        'conditions' in graph &&
        (graph as { mode?: string }).mode === 'form' &&
        Array.isArray((graph as { conditions?: unknown[] }).conditions)
    );
}

export default function StrategyRulesDisplay({
    graph,
}: {
    graph: FormModeGraph;
}) {
    return (
        <div className="mt-4 space-y-4">
            ...
            {graph.conditions.map(
                (group: ConditionGroup, groupIndex: number) => (
                    <div
                    ...
                    >
                        ...
                        <ul className="space-y-1 text-sm">
                            {group.rules.map(
                                (rule: StrategyRule, ruleIndex: number) => (
                                    <li
            ...
            <div className="rounded-md border p-3">
                ...
                <dl className="grid grid-cols-2 gap-1 text-sm">
                    ...
                    <dd>
                        {graph.risk.stoploss_pct !== null
                            ? `${graph.risk.stoploss_pct}%`
                            : 'Off'}
                    </dd>
                    <dt className="text-muted-foreground">Take Profit</dt>
                    <dd>
                        {graph.risk.take_profit_pct !== null
                            ? `${graph.risk.take_profit_pct}%`
                            : 'Off'}
                    </dd>
                    ...
                    <dd>
                        {graph.risk.daily_loss_limit_usdc !== null
                            ? `${graph.risk.daily_loss_limit_usdc} USDC`
                            : 'Off'}
                    </dd>
                    <dt className="text-muted-foreground">Cooldown</dt>
                    <dd>
                        {graph.risk.cooldown_seconds !== null
                            ? `${graph.risk.cooldown_seconds}s`
                            : 'Off'}
                    </dd>
